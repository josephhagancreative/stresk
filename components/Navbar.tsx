import React, { useState } from "react"
import Link from "next/link"
import Image from "next/future/image"

function Navbar() {
  const [navbar, setNavbar] = useState(false)

  return (
    <nav className="w-full bg-gradient-to-r from-purplebg to-blackbg fixed top-0 z-50">
      <div className="justify-between px-4 mx-auto lg:max-w-7xl md:items-center md:flex md:px-8">
        <div>
          <div className="flex items-center justify-between py-1 md:py-5 md:block">
            <Link href="/">
              <div>
                <Image
                  className=" cursor-pointer "
                  src={`/images/stresk-logo-sm-wt.png`}
                  width={75}
                  height={21}
                />
              </div>
            </Link>
            <div className="md:hidden">
              <button
                className="p-2 text-neutral-200 rounded-md outline-none focus:border-neutral-400 focus:none "
                onClick={() => setNavbar(!navbar)}>
                {navbar ? (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-6 h-6 hover:fill-neutral-300"
                    viewBox="0 0 20 20"
                    fill="white">
                    <path
                      fillRule="evenodd"
                      d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                      clipRule="evenodd"
                    />
                  </svg>
                ) : (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-6 h-6 hover:stroke-neutral-300"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="white"
                    strokeWidth={2}>
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M4 6h16M4 12h16M4 18h16"
                    />
                  </svg>
                )}
              </button>
            </div>
          </div>
        </div>
        <div>
          <div
            className={`flex-1 justify-self-center pb-3 mt-2 md:block md:pb-0 md:mt-0 ${
              navbar ? "block" : "hidden"
            }`}>
            <ul className="items-center justify-center text-center space-y-8 md:flex md:space-x-6 md:space-y-0">
              <li className="text-neutral-100 hover:text-neutral-300">
                <Link href="/">Home</Link>
              </li>
              <li className="text-neutral-100 hover:text-neutral-300 ">
                <Link href="/stretches">Stretches</Link>
              </li>
              <li className="text-neutral-100 hover:text-neutral-300">
                <Link href="/contact">Contact</Link>
              </li>
              <li className="text-neutral-100 hover:text-neutral-300">
                <Link href="/login">Login</Link>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </nav>
  )
}

export default Navbar
